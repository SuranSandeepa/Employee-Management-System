...
//Cancel btn
 ...
 //Register btn
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (Obadaella_VidyalayaEntities db = new Obadaella_VidyalayaEntities())
                    {
                        ...
                        {
                            db.Employee_Register.Add(modelR);
                        }
                        else//update
                        {
        ...
        void DisplayDataGridView()
        {
            ...
            using(Obadaella_VidyalayaEntities db = new Obadaella_VidyalayaEntities())
            {
                dgvRegister.DataSource = db.Employee_Register.ToList<Employee_Register>();
            }
        }
        ...
        private void dgvRegister_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    modelR.EMPNO = Convert.ToInt32(dgvRegister.CurrentRow.Cells["EMPNO"].Value); //select task record using EMPNO
            ...
            if(MessageBox.Show("Delete ? ", "Employee Registration",MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                using(Obadaella_VidyalayaEntities db = new Obadaella_VidyalayaEntities())
                {
                    ...
                    if (entry.State == EntityState.Detached)
                        db.Employee_Register.Attach(modelR);
