...
namespace Obadaella_Vidyalaya
{
    public partial class EmployeeSalary : Form
    {
        ...
        void Clear()
        {
            txtEmpPayID.Text = txtEmployeeID.Text = txtBrach.Text = txtAccount.Text = txtBasicSalary.Text = txtAllowances.Text = txtLoanInstallments.Text = txtTotalSalary.Text = null;
            btnSalSave.Text = "Save";
            btnSalDelete.Enabled = false;
            employeeSalaryDetail.EmpSalNo = 0;
            rbtnPeoples.Checked = false;
            rbtnNSB.Checked = false;
            rbtnBOC.Checked = false;
        }
        ...
        private void btnSalSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtEmpPayID.Text == string.Empty || txtEmployeeID.Text == string.Empty || txtBrach.Text == string.Empty || txtAccount.Text == string.Empty || txtBasicSalary.Text == string.Empty || txtAllowances.Text == string.Empty || txtTotalSalary.Text == string.Empty)
                {
                ...
                else
                {
                    ...
                    if (rbtnBOC.Checked)
                    {
                        employeeSalaryDetail.Bank = "BOC";
                    }
                    else if(rbtnNSB.Checked){
                        employeeSalaryDetail.Bank = "NSB";
                    }
                    else
                    {
                        employeeSalaryDetail.Bank = "Peoples Bank";
                    }
        ...
        private void dgvEmployeeSalary_DoubleClick(object sender, EventArgs e)
        {
            if (dgvEmployeeSalary.CurrentRow.Index != -1)
            {
                ...
                using (Obadaella_VidyalayaEntities db = new Obadaella_VidyalayaEntities())
                {
                    ...
                    if(employeeSalaryDetail.Bank == "BOC")
                    {
                        rbtnBOC.Checked = true;
                    }else if(employeeSalaryDetail.Bank == "NSB")
                    {
                        rbtnNSB.Checked = true;
                    }
                    else
                    {
                        rbtnPeoples.Checked = true;
                    }
